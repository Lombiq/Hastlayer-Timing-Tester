...
{
    /// <summary>
    /// Wraps another VHDL expression in SmartResize(), which is defined in <see cref="AdditionalVhdlIncludes"/>. The
    /// size parameter of SmartResize will be set to the input size. For example, those operators that normally required
    /// <see cref="VhdlOp.DoubleSizedOutput"/> can now work with <see cref="VhdlOp.SameOutputDataType"/> using
