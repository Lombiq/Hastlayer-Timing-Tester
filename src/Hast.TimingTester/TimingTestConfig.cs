...
/// <remarks>
/// <para>
/// You need to edit the source and recompile the application to change configuration. See the comments in the source of
/// this class for detailed description of the options. It is advised to check <see cref="Operators"/> first.
/// </para>
/// </remarks>
internal class TimingTestConfig
{
    ...
    {
        // There are lists of functions below that can generate input data types to test. Any of these can be used as a
        // parameter to the constructor of VhdlOp. (It is advised to start with the Operators variable when looking at
        // this file.) For example, for an input size of 32, we should get "unsigned(31 downto 0)" to be pasted into the
        // VHDL template. However, if a friendly name is requested instead, "unsigned32" is returned, which can be
        ...
        // advised to start with the Operators variable when looking at this file.)
        ...
        {
            new VhdlOp(
                new BinaryOperatorVhdlExpression("and"),
                "and",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("nand"),
                "nand",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("or"),
                "or",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("nor"),
                "nor",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("xor"),
                "xor",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("xnor"),
                "xnor",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("+"),
                "add",
                suNumericDataTypes,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                ...
                "gt",
                suNumericDataTypes,
                VhdlOp.ComparisonWithBoolOutput,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("<"),
                "lt",
            ...
            new VhdlOp(
                ...
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("mod"),
                "mod",
                suNumericDataTypes,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new BinaryOperatorVhdlExpression("rem"),
                "rem",
                suNumericDataTypes,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
            new VhdlOp(
                new UnaryOperatorVhdlExpression("not"),
                "not",
                stdLogicVectorDataType,
                VhdlOp.SameOutputDataType,
                defaultVhdlTemplates),
        ...
        // We test shifting by the amount of bits listed below. Multiplying by a constant 2^N is also a shift operation,
        // so we test that here, too. As we expect the FPGA compiler to implement this by wiring, multiplying by 2^N is
        ...
        // catalog file (https://github.com/Xilinx/XilinxBoardStore/tree/master/catalog) or check out the branch for the
        // given Vitis version, like this one for 2020.1.1:
        // https://github.com/Xilinx/XilinxBoardStore/tree/2020.1.1/boards/Xilinx. E.g. the part name for the Alveo U280
        // board is in the
        // https://github.com/Xilinx/XilinxBoardStore/blob/master/boards/Xilinx/au280/production/1.1/board.xml file,
        // just search for "part_name". Be sure to use the production versions, not the engineering sample ("es"). Use
        // the existing configurations under the TimingTestConfigs folder instead of directly changing this here, and
        // create new configs for new boards.
        ...
        // If DebugMode is true, the Hastlayer Timing Tester will stop at any exceptions during tests. If it is false,
        // the exceptions are logged and the program continues with the next test.
        DebugMode = false;
        ...
        // If VivadoBatchMode is true, Vivado shares the console window of Hastlayer Timing Tester. It does not open the
        // GUI for every single test. However, it cannot generate schematic drawings (Schematic.pdf). Note: if you are
        // using Vivado in GUI mode with VivadoBatchMode = false and with ImplementDesign = true, only generate designs
        // that are possible to implement, or a message box will pop up with Tcl errors, and the tests will hang.
        VivadoBatchMode = true;

        // If ImplementDesign is true, Vivado will perform STA for both the synthesized and the implemented designs. If
        // it is false, Vivado will only do synthesis + STA, and skip implementation + STA.
        ...
        // supports it. Currently only Vivado supports multi threading. This should most of the time correspond to the
        // number of logical processors (cores) in the system. Also see NumberOfSTAProcesses.
        ...
        // supposedly using all the cores the FPGA tools can't usually utilize the whole CPU you can increase the degree
        // of parallelism further. Also see NumberOfThreadsPerProcess.
