...
/// <summary>
/// Generates a VHDL expression for shifting left or right, but the expression it generates conforms the specification
/// of .NET shift operators. See this note from Hastlayer: "Contrary to what happens in VHDL binary shifting in .NET
/// will only use the lower 5 bits (for 32b operands) or 6 bits (for 64b operands) of the shift count. So e.g. 1
/// &lt;&lt; 33 won't produce 0 (by shifting out to the void) but 2, since only a shift by 1 happens (as 33 is 100001 in
/// binary). See: https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/left-shift-operator So we
/// need to truncate. Furthermore right shifts will also do a bitwise AND with just 1s on the count, see:
/// https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/right-shift-operator" It has two modes
/// of operation: shift by a constant or a variable number or bits, which can be set in the constructor.
/// </summary>
public class DotnetShiftVhdlExpression : VhdlExpressionBase
{
    ...
    /// Will ignore any test cases where the number of output bits does not equal this parameter. This filter can be
    /// switched off by setting this parameter to <see cref="NoOutputSizeCheck"/>.
    /// </param>
    ...
    /// the input size makes no sense, so we impose a restriction on this. We only work on test cases where input size
    /// and output size is the same, because of the complicated expression (including SmartResizes) in <see
    /// cref="GetVhdlCode"/>. This check can be however switched off, see <see cref="_outputSize"/> in the constructor.
    /// </summary>
    ...
    /// <summary>
    /// Used to generate a part of the expression in <see cref="GetVhdlCode"/>. It will generate a series of "1" in a
    /// string, the amount of which is log2(number of bits). It helps to achieve the dotnet way of shifting (e.g.
    /// shifting a 32-bit number by 33 will result in an 1-shift, see <see cref="DotnetShiftVhdlExpression"/> class
    /// definition docstring summary).
    /// </summary>
